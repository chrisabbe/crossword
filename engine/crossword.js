/* ===============================
  ...
  Improved:
  - tap any cell and start there
  - space/delete clears current cell
  - typing advances cell-by-cell
  - better across/down direction toggle
  =============================== */

const LS_KEY = (() => {
  const path = location.pathname.toLowerCase();

  const trialMatch = path.match(/trial-day\d+/);
  if (trialMatch) return "cw_" + trialMatch[0];

  const dayMatch = path.match(/day\d+/);
  if (dayMatch) return "cw_" + dayMatch[0];
...
function buildClues(p) {
  ...
  Object.keys(p.clues.across).map(Number).sort((a, b) => a - b).forEach(n => {
    ...
    d.onclick = () => activateWord("across", n, true, null, true);
  ...
  Object.keys(p.clues.down).map(Number).sort((a, b) => a - b).forEach(n => {
    ...
    d.onclick = () => activateWord("down", n, true, null, true);
...
function activateWord(dir, num, focus, targetCell = null, preferFirstEmpty = false) {
  clearHighlights();
  ...
  if (targetCell) {
    const idx = activeCells.findIndex(c => c.r === targetCell.r && c.c === targetCell.c);
    if (idx >= 0) {
      activeIndex = idx;
    }
  } else if (focus && preferFirstEmpty) {
...
function onCellTap(r, c) {
  ...
  const tappingSameActiveCell =
    lastTapCell.r === r &&
    lastTapCell.c === c &&
    activeCells[activeIndex] &&
    activeCells[activeIndex].r === r &&
    activeCells[activeIndex].c === c;

  // If tapping the same active cell again and it belongs to both directions, toggle.
  if (tappingSameActiveCell && hasAcross && hasDown) {
  ...
  activateWord(dir, num, true, { r, c }, false);
...
function handleKeyNav(e) {
  ...
  // Backspace = current behavior (clear current; if empty, move back and clear previous)
  if (e.key === "Backspace") {
    e.preventDefault();
  ...
  // Delete or Space = clear current cell only (useful for corrections)
  if (e.key === "Delete" || e.key === " " || e.key === "Spacebar") {
    e.preventDefault();
    cell.input.value = "";
    saveProgress();
    return;
  ...
  if (e.key.startsWith("Arrow")) {
    ...
    // Improve direction handling with arrows
    if (e.key === "ArrowRight") {
      if (activeDirection !== "across" && cell.acrossNum != null) {
        activateWord("across", cell.acrossNum, true, { r: cell.r, c: cell.c }, false);
      } else if (activeIndex < activeCells.length - 1) {
        activeIndex++;
        focusActiveCell();
      }
      return;
    ...
    if (e.key === "ArrowDown") {
      if (activeDirection !== "down" && cell.downNum != null) {
        activateWord("down", cell.downNum, true, { r: cell.r, c: cell.c }, false);
      } else if (activeIndex < activeCells.length - 1) {
        activeIndex++;
        focusActiveCell();
      }
